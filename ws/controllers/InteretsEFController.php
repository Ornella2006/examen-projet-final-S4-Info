...
class InteretsEFController {
    ...
    public function showSynthese() {
        ...
        $sql = "SELECT DATE_FORMAT(date_remboursement, '%Y-%m') AS periode, SUM(interet) AS total_interets 
                FROM Remboursement_EF 
                WHERE date_remboursement BETWEEN :date_debut AND :date_fin 
                GROUP BY periode 
                ORDER BY periode";
